...
var pinMapKey = "edison";
var pinMap = pinMapping[pinMapKey];
...
if (Pin.IO.getPlatformType() === 2) {
  ...
  if (isMiniboard) {
    ...
    // Override the default pin mapping
    pinMapKey = "arduino";
    pinMap = pinMapping[pinMapKey];
  }
}
...
function Galileo(opts) {
  ...
  var awaiting = [];
  var state = {
    adc: opts.adc !== undefined ? opts.adc : null,
    i2c: opts.i2c !== undefined ? opts.i2c : null,
  ...
  if (opts.map !== undefined) {
    ...
    if (pinMapKey === "dfrobotromeo") {
      pinModes = [].concat.call(pinModes, pmCopy.slice(-5));
    }
  }
  ...
  this.analogPins = this.pins.slice(-5).map(function(pin, i) {
    return i;
  });
...
var edisonArduinoPinError = "Edison Arduino Board has no connection at pin ";
var edisonMiniPinError = "Edison Mini Board has no connection at pin ";


function ToPinIndex(pin) {
  var offset = pin[0] === "A" ? 14 : 0;
  var hasAnalog = pinModes.some(function(pin) {
    return pin && pin.analogChannel !== undefined;
  });
  var index;

  if (!isMiniboard) {
    if (typeof pin === "number" && pin > 19) {
      throw new Error(edisonArduinoPinError + pin);
    }
  }

  // Cases where an expansion is detected as being a miniboard.
  if (isMiniboard && !hasAnalog) {
    offset = 0;

    if (typeof pin === "number" && pinModes[pin] === null) {
      throw new Error(edisonMiniPinError + pin);
    }

    index = pinMap[String(pin).toUpperCase()];
  ...
  // Cases where an expansion is detected as being a miniboard,
  // but provides analog read capabilities by some other means.
  if (isMiniboard && hasAnalog) {
    index = pinMap[pin.toUpperCase()];
    return index;
  }
...
Galileo.prototype.pinMode = function(pin, mode) {
  ...
  if (isMiniboard) {
    ...
    if (mode === 2 && state.map !== "dfrobotromeo") {
...
Galileo.prototype.analogRead = function(pin, handler) {
  ...
  isGpio = this.pins[pinIndex] !== undefined;

  if (!isGpio) {
    offset = this.pins.findIndex(function(pin) {
      return pin.analogChannel !== undefined;
    });

    rawIndex = pinIndex;
    pinIndex = pinIndex - 160 + offset;
  }
  ...
  // DFRobot Romeo uses an Atmega8 to provide ADC.
  // Hopefully libmraa will implement support.

  if (state.adc !== null && state.adc.type === "i2c" && !state.adc.isActive) {
    state.adc.isActive = true;
    ...
    state.adc.i2c = new Pin.IO.I2c(state.adc.bus);
    state.adc.i2c.address(state.adc.address);

    setInterval(function() {
      var buffer = new Buffer(state.adc.i2c.read(12));

      if (buffer.length === 12) {
        for (var i = 0; i < 6; i++) {
          var report = reporting[i + 160];
          var offset = i * 2;

          if (report) {
            processRead(this, report, buffer.readUInt16LE(offset));
          }
        }
      }
    }.bind(this), 19);
