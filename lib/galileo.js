...
if (mraa) {
  ...
  Galileo.prototype.i2cWrite = function(address, cmdRegOrData, inBytes) {
    ...
    bytes = [cmdRegOrData].concat(inBytes);
    ...
    if (bytes.length) {
      ...
      if (result !== mraa.MRAA_SUCCESS) {
        // console.log("Batch failed");
        //
        for (var i = 0; i < bytes.length; i++) {
          result = state.i2c.write(new Buffer(bytes[i]).toString());
          // if (result !== mraa.MRAA_SUCCESS) {
          //   console.log("Single Byte failed");
          // }
        }
      }
    }
